...
// 仮の弾をスポーン（初速なし）
    public BulletController SpawnPreviewBullet(Vector2 position) {
        BulletData newBulletData = new BulletData(position, Vector2.zero, bulletRadius);
        ...
        if (controller != null) {
            ...
            return controller; // 管理にはまだ追加しない
        }
        Debug.LogError("BulletPrefab に BulletController が必要です。");
        return null;
    }

// Preview弾を本物の弾として登録
    public void RegisterBullet(BulletController controller) {
        if (controller != null && !bulletControllers.Contains(controller))
        {
            bulletControllers.Add(controller);
            controller.SetNumber(bulletControllers.Count);
        }
    }
